...
매번 수작업하는 것이 아니라 자동화하는 것이 좋다.

### 변경 내용 검사

소스 트래킹 도구로 깃을 사용한다면 깃 훅을 사용하는 것이 좋다. 깃 커맨드 실행 시점에 끼어들 수 있는 훅을 제공한다. `husky`는 깃 훅을 쉽게 사용할 수 있도록 도와주는 도구이다. commit message를 작성하기 이전에 린트로 코드를 검사하는 작업을 추가한다.
...
만약 린트 검사를 수행중에 오류를 발견하게 되면 커밋은 취소된다.

<br>

코드양이 많아지면 커밋전에 모든 코드를 린트로 검사하게 되면 많은 시간이 소요될 것이다.

**변경된 파일만** 린트로 검사하게 할 수 있다.

`lint-staged`를 사용하여 변경된(스테이징된) 파일만 린트를 수행하게 할 수 있다.
...
이제 모든 파일을 검사하는 것이 아니라 변경 혹은 추가된 파일만 검사한다.

<br>

### 에디터 확장도구

코딩할 때 실시간으로 검사하는 방법이 있다.

vscode의 eslint와 prettier 익스텐션이 기능을 제공한다.

프리티어 규칙을 eslint에 통합했기 때문에 eslint 익스텐션을 사용하면 충분하다.
